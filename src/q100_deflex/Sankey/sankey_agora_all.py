...
url = ("https://files.de-1.osf.io/v1/resources/4nfam/providers/osfstorage"
...
year = 2018
sankey_in["Heat 2018"] = heat_2018(path, f"{year}-DE02-Agora.dflx", False)
sankey_in["Mob-H2 2018"] = mobh2_2018(path, f"{year}-DE02-Agora.dflx", False)
sankey_in["Overview 2018"] = overv_2018(path, f"{year}-DE02-Agora.dflx", False)
sankey_in["Power 2018"] = power_2018(path, f"{year}-DE02-Agora.dflx", False)

year = 2030
sankey_in["Heat 2030"] = heat_2030(path, f"{year}-DE02-Agora.dflx", False)
sankey_in["Mob-H2 2030"] = mobh2_2030(path, f"{year}-DE02-Agora.dflx", False)
sankey_in["Overview 2030"] = overv_2030(path, f"{year}-DE02-Agora.dflx", False)
sankey_in["Power _2030"] = power_2030(path, f"{year}-DE02-Agora.dflx", False)

year = 2050
sankey_in["Heat 2050"] = heat_2050(path, f"{year}-DE02-Agora.dflx", False)
sankey_in["Mob-H2 2050"] = mobh2_2050(path, f"{year}-DE02-Agora.dflx", False)
sankey_in["Overview 2050"] = overv_2050(path, f"{year}-DE02-Agora.dflx", False)
sankey_in["Power 2050"] = power_2050(path, f"{year}-DE02-Agora.dflx", False)

filename = os.path.join(q100_path, "sankey", "sankey_input.xlsx")
os.makedirs(os.path.dirname(filename), exist_ok=True)
dflx.dict2file(sankey_in, filename)
...
holoviews_sankey.main(filename, **kwargs)
