...
HeatPlant=np.sum(comm.xs("heat plant", level=4, axis=1)) # All heat plant
ComHeatPlantbio=HeatPlant[0]
ComHeatPlantcoal=HeatPlant[1]
ComHeatPlantgas=HeatPlant[2]
ComHeatPlantother=HeatPlant[3]

DecHeat=np.sum(comm.xs("decentralised heat", level=4, axis=1)) # All decnetralized heat technologies
...
ShortageHPDHheat=np.sum(np.sum(dh.xs("shortage", level=0, axis=1))) # this is the shortage in MW_th
ShortageHPDHelectr=ShortageHPDHheat/1.88 #this is the shortage in MW_el
...
EbusHPDH=np.sum(np.sum(elect.xs("heat pump DH", level=5, axis=1)))+ShortageHPDHelectr
...
decheat=np.sum(decentralisedheat.xs("decentralised heat", level=0, axis=1))
...
["Heat Pump decentralised", "Dec. Heat Demand", DecHeatheatpumpDecHeatdemand],
...
a=create_and_save_sankey(df,
                         filename=os.path.join(path, "Sankey Deflex - 2030 Heat Sector"),
                         title="Heat Sector - 2030",
                         edge_color_index="To",
                         fontsize=20,
